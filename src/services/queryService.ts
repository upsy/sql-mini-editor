...
export const fetchQueryHistoryService = (bpmEngine: BPMEngine, url: string): Promise<QueryHistoryItem[]> => {
    ...
    const fetchQueryHistoryPromise: Promise<QueryHistoryItem[]> = new Promise((resolve, reject) => {
        callBpmAjax(bpmEngine, url, inputJSON, (data) => {
    ...
    toast.promise(fetchQueryHistoryPromise, {
        loading: 'Retrieving Historical Queries For Inspiration...',
        success: () => {
            return `Success! Queries Retrieved!`;
        },
        error: 'Error!',
    });


    return fetchQueryHistoryPromise;
};
...
export const fetchDbSchemaService = (bpmEngine: BPMEngine, url: string): Promise<TableSchema[]> => {
    ...
    toast.promise(fetchDbSchemaService, {
        loading: 'Initializing Dashboard! Retrieving DB Schema configurations...',
        success: () => {
